...
class SearchedEvents extends React.Component  {
  componentDidMount() {
    // props
    const { fetchEvents, handleQuickSearch, handleSubmit, addFilters } = this.props;
              ...
              
              {
                (() => {
                  if (homeFormData.length < 0 ) {
                    ...
                    return (
                      <>
                      {homeFormData.length > 0 ? homeFormData.map((event) => <Event key={event.id} {...event} /> ) : <p>Aucun événement ne correspond à votre recherche</p>}                                  
